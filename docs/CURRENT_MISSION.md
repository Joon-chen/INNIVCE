...
- Cognitive Foundation V1.2：Company Snapshot 样例链已升级为 `Shared File Intelligence -> Evidence Pack -> Evidence -> Candidate -> Snapshot Builder -> Snapshot -> Runtime`；Evidence Pack 是通用证据结构，只保存 source ref、content profile、outline、claims、entities、topics、relations、metrics、spans、uncertainties、quality 和 derived_from，不允许出现 `company_* / customer_* / project_*` 业务字段；Company Extractor 只消费 Evidence Pack 并输出 Candidate，Snapshot payload 仍固定为 `identity / structured / understanding / evidence_refs / confidence / version / snapshot_status / derived_from`。Snapshot Builder 可在后台使用 LLM 优化 `understanding`，失败时回退确定性认知，不允许 Runtime 每次重新总结 PDF。V1.2 不启动长期 Memory，也不把 Snapshot 扩成第二个 Knowledge Database。
- Organization Foundation：通讯录和组织架构作为基础数据必须可信输出；部门成员、负责人和人员字段不得把数字标识、工号、open_id 当作姓名展示。部门结果后的“谁是领导/负责人”会继承上一轮部门对象，转成 `department_members + organization_relation=leader`，不会把“谁”当人员关键词搜索。
