...
- Cognitive Foundation V1.1：Company Snapshot 样例链已冻结为 `Evidence -> Candidate -> Snapshot Builder -> Snapshot -> Runtime`；Extractor 只输出 Candidate，Snapshot payload 使用 `identity / structured / understanding / evidence_refs / confidence / version / snapshot_status / derived_from`，Runtime 优先消费 Snapshot，不再把公司画像问答退回 PDF 临场总结或 Knowledge Chunk 摘要。V1.1 不启动长期 Memory，也不把 Snapshot 扩成第二个 Knowledge Database。
