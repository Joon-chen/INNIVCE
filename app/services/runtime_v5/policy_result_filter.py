...
def build_policy_result_filter_payload(
    *,
    ...
    resource_filters = tuple(
        _resource_filter(index=index, item=item, allowed=permission.allowed, scope=scope)
        for index, item in enumerate(result_context.items if result_context is not None else ())
    )
    section_filters = {
        resource_type: _section_filter(resource_type=resource_type, allowed=permission.allowed, scope=scope)
...
def _resource_filter(*, index: int, item: dict[str, Any], allowed: bool, scope: str) -> dict[str, Any]:
    resource_type = _resource_type(item)
    cognitive = resource_type in COGNITIVE_RESOURCE_TYPES
    aggregate = cognitive and scope in AGGREGATE_SCOPES
    return {
        "index": index,
        ...
        "source_reference_visible": allowed and not aggregate,
    }


def _section_filter(*, resource_type: str, allowed: bool, scope: str) -> dict[str, Any]:
    cognitive = resource_type in COGNITIVE_RESOURCE_TYPES
    aggregate = cognitive and scope in AGGREGATE_SCOPES
    return {
        "visible": allowed,
